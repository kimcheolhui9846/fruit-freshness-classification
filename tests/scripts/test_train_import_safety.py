...
class TrainImportSafetyTest(unittest.TestCase):
    def test_import_has_no_dataset_requirement_or_filesystem_side_effect(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(
                [sys.executable, "-c", "import scripts.train; print('import safe')"],
                cwd=directory,
                env=environment,
                capture_output=True,
                text=True,
    ...
    def test_canonical_help_command_neither_trains_nor_creates_output(self):
        result = subprocess.run(
