...
The two verification runs executed on 2026-08-15 and returned `A_ADOPTED`, the ladder's strongest branch: level `A_STRICT` completed both runs with no nondeterministic-operation error, and the model weights, EMA weights, and fold histories were bit-exact. `sha256sum` over the four checkpoint files each run wrote confirmed the match independently of the project's own comparison script. No descent to `B_CUDNN` was required.
