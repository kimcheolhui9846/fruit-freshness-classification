...
- **The holdout overlaps the training data.** 1,618 of its 5,372 rows (30.1%) are byte-identical copies of training images, because the source dataset duplicates files and the split is by row rather than by unique image. On the rows without such a copy, Top-1 is 0.9414 rather than the reported 0.9555. The reported figures are unrevised; see the [dataset duplication audit](dataset-duplication-audit.md). The `freshpotato` and `rottenpotato` classes contain no duplicates, so the class-specific findings above are unaffected.
